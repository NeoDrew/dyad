...
import static dyad.testutil.FormUtil.getCsrfToken;
...
@SpringBootTest(classes = Dyad.class, webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
@ActiveProfiles("test")
public class SecurityIntegrationTest {
	...
	public void signOut() throws Exception {
		...
		// GET the stocks page so we can read the CSRF token for the logout form.
		result = client.get().uri("/stocks").accept(MediaType.TEXT_HTML).cookie(SESSION_KEY, cookie).exchange()
				.expectStatus().isOk().expectBody(String.class).returnResult();
