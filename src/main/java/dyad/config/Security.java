...
public class Security extends WebSecurityConfigurerAdapter {
	...
	public UserDetailsService userDetailsService() {
		...
		UserDetails andrew = User.withUsername("AM").password(encoder.encode("Admin")).roles(ADMIN_ROLE).build();
		UserDetails gerald = User.withUsername("Gerald").password(encoder.encode("Admin")).roles(ADMIN_ROLE).build();

		return new InMemoryUserDetailsManager(andrew, gerald);
	}
}
